...
export async function apply(ctx) {
  ...
  // path). Read lazily per request, never captured here: the service starts
  // late (it waits on storageDomain + sessionPersistence bootstrap), so a
  // capture at apply time froze `undefined` and every /workspaces answer
  // reported an empty registry — sidebar rows then degraded to "Workspace
  // path not found." Absent in stripped-down hosts the route still degrades
  // to an empty list rather than blocking the session-header actions.
  const workspaceList = () => {
    const registry = ctx.get('workspaceRegistry')
    return registry && typeof registry.list === 'function' ? registry.list() : []
  }
