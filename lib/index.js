...
/**
 * Validate a client-supplied directory path: absolute, bounded length, no NUL,
 * resolvable. Returns the resolved absolute path or null.
 */
...
/** The path must exist and be a directory before any launcher runs. */
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (!entry || !entryApplies(entry)) {
        return sendJson(res, 400, { ok: false, error: 'unknown target: ' + targetId })
      }
      const path = normalizeTargetPath(body && body.path)
      if (path === null) {
        return sendJson(res, 400, { ok: false, error: 'path must be an absolute directory path' })
      }
      try {
        await assertDirectory(path)
      } catch {
        return sendJson(res, 400, { ok: false, error: 'not a directory: ' + path })
      }
      const detected = (await detectTargets()).find((t) => t.id === targetId)
      if (!detected || !detected.available || !detected.bin) {
        return sendJson(res, 400, { ok: false, error: 'launcher not available: ' + entry.label })
      }
      const result = await launch(detected.bin, entry.args(path), entry.cwdBased ? path : undefined)
      sendJson(res, result.ok ? 200 : 500, { ok: result.ok, target: targetId, path, error: result.error })
    },
